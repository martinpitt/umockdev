...
class Testbed(unittest.TestCase):
    ...
    def run_mainloop(self, mainloop, timeout=500):
        '''Run the main loop for a given timeout in milliseconds.'''
        GLib.timeout_add(timeout, mainloop.quit)
        with warnings.catch_warnings():
            # happens with Python 3.14 and GLib 2.85.1
            warnings.filterwarnings(
                "ignore",
                category=DeprecationWarning,
                message=r".*asyncio\.(AbstractEventLoopPolicy|get_event_loop_policy).*"
            )
            mainloop.run()
