...
public class Testbed: GLib.Object {
    ...
    public bool add_from_string (string data) throws UMockdev.Error
    {
        ...
        if (this.re_dump_val == null)
            this.re_dump_val = /^([PS]): (.*)(?>\n|$)/;
        if (this.re_dump_keyval == null)
            this.re_dump_keyval = /^([EAH]): ([a-zA-Z0-9_:+-]+)=(.*)(?>\n|$)/;
        if (this.re_dump_optval == null)
            this.re_dump_optval = /^([N]): ([^=\n]+)(?>=([0-9A-F]+))?(?>\n|$)/;
    ...
    private string add_dev_from_string(string data) throws UMockdev.Error
    {
        ...
        cur_data = this.dump_parse_line(cur_data, out type, out key, out devpath);
        ...
        while (cur_data.length > 0 && cur_data[0] != '\n') {
            cur_data = this.dump_parse_line(cur_data, out type, out key, out val);
            if (cur_data == null)
     ...
     * is not valid.
     */
    private string? dump_parse_line (string data, out char type, out string? key, out string? val)
    {
        MatchInfo match;

        if (this.re_dump_val.match(data, 0, out match)) {
            key = null;
            val = match.fetch(2);
        } else if (this.re_dump_keyval.match(data, 0, out match)) {
            key = match.fetch(2);
            val = match.fetch(3);
            assert(val != null);
        } else if (this.re_dump_optval.match(data, 0, out match)) {
            key = match.fetch(2);
            val = match.fetch(3);
        } else {
            debug("dump_parse_line: >%s< does not match anything, failing", data);
            type = '\0';
            key = null;
            val = null;
            return null;
    ...
    private Regex re_dump_val;
    private Regex re_dump_keyval;
    private Regex re_dump_optval;
